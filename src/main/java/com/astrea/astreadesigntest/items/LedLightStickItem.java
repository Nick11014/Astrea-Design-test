...
import com.astrea.astreadesigntest.client.renderer.LedLightStickRenderer;
import com.astrea.astreadesigntest.sounds.ModSounds;

import java.util.List;
import java.util.function.Consumer;

public class LedLightStickItem extends Item implements GeoItem {
    //private static final RawAnimation ACTIVATE_ANIM = RawAnimation.begin().thenPlay("side_button_activate");
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public LedLightStickItem(Properties properties) {
        super(properties);
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private LedLightStickRenderer renderer;

            @Override
            public BlockEntityWithoutLevelRenderer getGeoItemRenderer() {
                if (this.renderer == null)
                    this.renderer = new LedLightStickRenderer();
